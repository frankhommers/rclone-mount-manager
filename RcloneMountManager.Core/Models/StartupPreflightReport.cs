...
public sealed class StartupPreflightReport
{
  ...
  public StartupPreflightReport AddPass(string checkKey, string message)
  {
    return Add(StartupCheckResult.Pass(checkKey, message));
  }

  public StartupPreflightReport AddWarning(string checkKey, string message)
  {
    return Add(StartupCheckResult.Warning(checkKey, message));
  }

  public StartupPreflightReport AddCritical(string checkKey, string message)
  {
    return Add(StartupCheckResult.Critical(checkKey, message));
  }
  ...
  public string ToSummaryText()
  {
    ...
    int passed = _checks.Count(check => check.IsPass);
    int warnings = _checks.Count(check => check.IsWarningFailure);
    int critical = _checks.Count(check => check.IsCriticalFailure);
