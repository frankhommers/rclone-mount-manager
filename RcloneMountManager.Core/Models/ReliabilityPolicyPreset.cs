  ...
  IReadOnlyDictionary<string, string> OptionOverrides)
{
  ...
  public static IReadOnlySet<string> ManagedReliabilityKeys { get; } =
  ...
  {
    if (!string.IsNullOrWhiteSpace(presetId) && CatalogById.TryGetValue(presetId, out ReliabilityPolicyPreset? preset))
    {
      return preset;
    }
  ...
  {
    Dictionary<string, ReliabilityPolicyPreset> byId = new(StringComparer.OrdinalIgnoreCase);
    foreach (ReliabilityPolicyPreset preset in Catalog)
    {
