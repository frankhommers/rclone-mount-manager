...
public sealed class RcloneRcClient
{
  ...
  public async Task VfsRefreshAsync(int rcPort, string? dir, CancellationToken cancellationToken)
  {
    using HttpRequestMessage request = new(HttpMethod.Post, $"http://localhost:{rcPort}/vfs/refresh");
    string body = string.IsNullOrEmpty(dir)
      ? "{}"
      : JsonSerializer.Serialize(new {dir});
