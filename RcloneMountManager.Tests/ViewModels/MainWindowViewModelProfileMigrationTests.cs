...
{
    private readonly string _tempRoot = Path.Combine(Path.GetTempPath(), $"main-window-profile-migration-tests-{Guid.NewGuid():N}");
    ...
    public void LoadProfiles_MigratesLegacyRcMountOptions_ToRcPortAndRemovesLegacyKeys()
    {
        ...
        var profile = Assert.Single(viewModel.Profiles, static p => !p.IsRemoteDefinition);
